...
from Modules.PyHydro.hidr import hidr
...
class submercado(object):
                ...
                for iusina in usinas:
                    if iusina.Sist == self.Codigo:
                        if iusina.TipoReg == b'M':
                            self.EC[iano, imes] = self.EC[iano, imes] + iusina.RoAcum[0, 0]*iusina.QIncHistEntreRes(usinas, iano, imes)
                            # self.EC2[iano, imes] = self.EC2[iano, imes] + iusina.RoAcumFiodAguaEquiv[0, 0]*iusina.Vazoes[iano, imes]
                        else:
                            self.EFIO[iano, imes] = self.EFIO[iano, imes] + iusina.RoEquiv[0, 0]*iusina.QIncHistEntreRes(usinas, iano, imes)
            ...
            for imes in range(12):
                ...
                for iusina in usinas:
                    if iusina.TipoReg == b'M' and iusina.Sist == self.Codigo and iusina.StatusVolMorto[iano][imes] == 2:
                        self.EAMAX[iano][imes] = self.EAMAX[iano][imes] + (1/FATOR) * iusina.RoAcum[iano][imes]*iusina.VolUtil
    ...
    def CalcEArmMin (self, usinas:List[hidr]):
        ...
        for iano in range(5):
            for imes in range(12):
                ...
                for iusina in usinas:
                    if iusina.TipoReg == b'M' and iusina.Sist == self.Codigo and iusina.StatusVolMorto[iano][imes] == 2:
                        self.EAMIN[iano][imes] = self.EAMIN[iano][imes] + (1 / FATOR) * iusina.RoAcumOperEquiv[iano][imes] * (iusina.VolMinP[iano, imes] - iusina.VolMin)
    ...
    def CalcEArmMed (self, usinas:List[hidr], ano_ini):
        ...
        for iano in range(5):
            for imes in range(12):
                ...
                for iusina in usinas:
                    if iusina.TipoReg == b'M' and iusina.Sist == self.Codigo and iusina.StatusVolMorto[iano][imes] == 2:
                        self.EAMED[iano][imes] = self.EAMED[iano][imes] + (1/FATOR) * iusina.RoAcum50[iano][imes]*iusina.VolUtil*0.5
    ...
    def CalcParamFC(self, usinas:List[hidr]):
        ...
        for imes in range(12):
            ...
            if self.EAMAX[0, 0] == 0 and self.EAMIN[0, 0] == 0 and self.EAMED[0, 0] == 0:
                ...
                param = np.polyfit(np.array([0, self.EAMED[0, 0], self.EAMAX[0, 0]]), np.array([fcmin, fcmed, fcmax]), deg=2)
    ...
    def CalcParamEVMin(self, usinas:List[hidr]):
        ...
        for imes in range(12):
            ...
            if self.EAMAX[0, 0] == 0 and self.EAMIN[0, 0] == 0 and self.EAMED[0, 0] == 0:
                ...
                param = np.polyfit(np.array([0, self.EAMED[0, 0], self.EAMAX[0, 0]]), np.array([evmin_min, evmin_med, evmin_max]), deg=2)
            ...
            for imes in range(12):
                for iusina in usinas:
                    if iusina.Sist == self.Codigo:
                        ...
                        if iusina.TipoReg != b'M':
                            self.EDESVF[iano][imes] += iusina.RoAcumFiodAguaEquiv[iano, imes] * iusina.VazDesv[iano, imes]
    ...
    def CalcParamGHMAX(self, usinas:List[hidr]):
        ...
        if self.EAMAX[0, 0] == 0 and self.EAMIN[0, 0] == 0 and self.EAMED[0, 0] == 0:
            ...
            param = np.polyfit(np.array([0, self.EAMED[0, 0], self.EAMAX[0, 0]]), np.array([GHMAX_min, GHMAX_med, GHMAX_max]), deg=2)
    ...
    def CalcParamEVP(self, usinas:List[hidr]):
        ...
        for imes in range(12):
            ...
            if self.EAMAX[0, 0] == 0 and self.EAMIN[0, 0] == 0 and self.EAMED[0, 0] == 0:
                ...
                param = np.polyfit(np.array([0, self.EAMED[0, 0], self.EAMAX[0, 0]]), np.array([evp_min, evp_med, evp_max]), deg=2)
            ...
            if iusina.Sist == self.Codigo:
                for iano in range(5):
                    for imes in range(12):
                        # FATOR = 24 * 60 * 60 * monthrange(ano_ini+iano, imes + 1)[1] / 1000000
                        FATOR = 2.63
                        self.EVM[iano, imes] += (1/FATOR) * iusina.VolMortoTempo[iano, imes] * iusina.RoAcum[iano, imes]
    ...
    def PlotaMercado(self):
        ...
        if self.Ficticio == 0:
            ...
            ax.plot(np.arange(1, nr_lin*nr_col+1) , carga.reshape( nr_lin*nr_col, ), 'r-',lw=3, label = 'Mercado Total')
            ax.plot(np.arange(1, nr_lin*nr_col+1) , pq.reshape( nr_lin*nr_col, ), 'g--',lw=2, label = 'Usinas Nao Simuladas')
            ax.fill_between(np.arange(1, nr_lin*nr_col+1), 0, pq.reshape( nr_lin*nr_col, ), facecolor='g', alpha=0.1)
            ax.fill_between(np.arange(1, nr_lin*nr_col+1), pq.reshape( nr_lin*nr_col, ), carga.reshape( nr_lin*nr_col, ), facecolor='r', alpha=0.1)
