...
class hidr(object):
    ...
    def CalcProdutibs(self, nanos):       # Calcula Produtibilidades Associadas aa diversos volumes
        ...
        if (self.VolUtil > 0):
            ...
            for i in range(5):
                cota = cota + self.PolCotaVol[i] * (self.VolMax**(i+1)) / (i+1) - self.PolCotaVol[i] * (self.VolMin**(i+1)) / (i+1)
                cota65 = cota65 + self.PolCotaVol[i] * (Vol65**(i+1)) / (i+1) - self.PolCotaVol[i] * (self.VolMin**(i+1)) / (i+1)
                cota50 = cota50 + self.PolCotaVol[i] * (Vol50 ** (i + 1)) / (i + 1) - self.PolCotaVol[i] * (self.VolMin ** (i + 1)) / (i + 1)
                cota_minop = cota_minop + self.PolCotaVol[i] * (self.VolMinP[1, 0] ** (i + 1)) / (i + 1) - self.PolCotaVol[i] * (self.VolMin ** (i + 1)) / (i + 1)
            cota = cota / self.VolUtil
            ...
            for imes in range(12):
                ...
                for ilag in range(int(self.Ordem[imes])):
                    ...
                    if mes_ant < 0:
                        ano_ant -= 1
                        mes_ant += 12
                    residuos[iano-1][imes] -= self.CoefParp[imes][ilag] * (self.Vazoes[ano_ant][mes_ant] - media[mes_ant]) / desvio[mes_ant]
    ...
    def parp_alexandre(self, ord_max):
        ...
        media = np.mean(self.Vazoes[:nanos, :], axis=0)    # A primeira serie historica eh utilizada como tendencia (despreze-a)
        desvio = np.std(self.Vazoes[:nanos, :], axis=0)    # A primeira serie historica eh utilizada como tendencia (despreze-a)
            ...
            for ilag in range(ord_max):
                for iano in np.arange(1, nanos):
                    ...
                    if mes_ant < 0:
                        ano_ant -= 1
                        mes_ant += 12
                    FAC[ilag] += (self.Vazoes[iano][imes] - media[imes]) * (self.Vazoes[ano_ant][mes_ant] - media[mes_ant])
            ...
            for iano in np.arange(1, nanos):
                ...
                for ilag in range(int(self.Ordem[imes])):
                    ...
                    if mes_ant < 0:
                        mes_ant += 12
                        ano_ant -= 1
                    somatorio -= self.CoefParp[imes, ilag] * ((self.Vazoes[ano_ant, mes_ant] - media[mes_ant]) / desvio[mes_ant])
    ...
    def Teste_Residuo_ARCH(self,residuos):
        ...
        for iteste in range(nestagios):
            a = residuos[:,cont]
            t_valor,p_valor, p1, p2 = diagnostic.het_arch(a, maxlag=20, autolag=None, store=False, regresults=False, ddof=0)
            Teste[1,iteste] = p_valor*100
