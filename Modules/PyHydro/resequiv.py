...
class resequiv(object):
            ...
            for imes in range(12):
                for iusina in usinas:
                    if iusina.VolUtil > 0 and iusina.Ree == self.Codigo and iusina.StatusVolMorto[iano][imes] == 2:
                        self.EAMAX[iano][imes] = self.EAMAX[iano][imes] + iusina.RoAcum[iano][imes]*iusina.VolUtil/2.63
    ...
    def CalcEArmMed(self, usinas:List[hidr]):
        ...
        for iano in range(5):
            for imes in range(12):
                for iusina in usinas:
                    if iusina.VolUtil > 0 and iusina.Ree == self.Codigo and iusina.StatusVolMorto[iano][imes] == 2:
                        self.EAMED[iano][imes] = self.EAMED[iano][imes] + iusina.RoAcum[iano][imes]*iusina.VolUtil*0.65/2.63
                ...
                for iusina in usinas:
                    if iusina.Ree == self.Codigo:
                        if iusina.TipoReg == b'M':
                            ...
                            self.EFIOB[iano, imes] = self.EFIOB[iano, imes] + iusina.RoEquiv[0, 0] * iusina.QIncHistEntreRes(usinas, iano, imes)
    ...
    def CalcParamFC(self, usinas:List[hidr]):
        ...
        if self.EAMAX[0, 0] == 0 and self.EAMIN[0, 0] == 0 and self.EAMED[0, 0] == 0:
            ...
            param = np.polyfit(np.array([self.EAMIN[0, 0], self.EAMED[0, 0], self.EAMAX[0, 0]]), np.array([fcmin, fcmed, fcmax]), deg=2)
    ...
    def CalcParamEVMin(self, usinas:List[hidr]):
        ...
        if self.EAMAX[0, 0] == 0 and self.EAMIN[0, 0] == 0 and self.EAMED[0, 0] == 0:
            ...
            param = np.polyfit(np.array([self.EAMIN[0, 0], self.EAMED[0, 0], self.EAMAX[0, 0]]), np.array([evmin_min, evmin_med, evmin_max]), deg=2)
    ...
    def CalcParamEVP(self, usinas:List[hidr]):
        ...
        for imes in range(12):
            ...
            for iusina in usinas:
                if iusina.Ree == self.Codigo:
                    area_max = iusina.PolCotaArea[0] + iusina.PolCotaArea[1]*iusina.CotaMax + iusina.PolCotaArea[2]*iusina.CotaMax**2 + \
                               iusina.PolCotaArea[3]*iusina.CotaMax**3 + iusina.PolCotaArea[4]*iusina.CotaMax**4
                    area_min = iusina.PolCotaArea[0] + iusina.PolCotaArea[1] * iusina.CotaMin + iusina.PolCotaArea[2] * iusina.CotaMin ** 2 + \
                               iusina.PolCotaArea[3] * iusina.CotaMin ** 3 + iusina.PolCotaArea[4] * iusina.CotaMin ** 4
                    cota_med = iusina.PolCotaVol[0] + iusina.PolCotaVol[1] * (iusina.VolUtil*0.65) + iusina.PolCotaVol[2] * (iusina.VolUtil*0.65)**2 +\
                               iusina.PolCotaVol[3] * (iusina.VolUtil * 0.65)**3 + iusina.PolCotaVol[4] * (iusina.VolUtil * 0.65) ** 4
                    area_med = iusina.PolCotaArea[0] + iusina.PolCotaArea[1] * cota_med + iusina.PolCotaArea[2] * cota_med ** 2 + \
    ...
    def PlotaEArmMax(self, usinas:List[hidr]):
        ...
        for iano in range(nanos):
            for imes in range(12):
                for iusina in usinas:
                    if iusina.VolUtil > 0 and iusina.Ree == self.Codigo and iusina.StatusVolMorto[iano][imes] == 2:
                        FioJusante[iano][imes] = FioJusante[iano][imes] + iusina.RoAcum_B_Ree[iano][imes]* iusina.VolUtil / 2.63
                        ContJusante[iano][imes] = ContJusante[iano][imes] + iusina.RoAcum_C_Ree[iano][imes]* iusina.VolUtil / 2.63
